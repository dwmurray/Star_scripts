...
def getRadialProfile_py(pf, Particle_attributes, ParticleID, creation_time, current_time, fileout="rad_profile.out", radiusMin=1e-3, radiusSphere=3.0, ParticleMass = 0.0) : 
	...
	print "Particle position: ", xc, yc, zc
	print "Particle velocity: ", vxc, vyc, vzc
	print "Particle Spin momentum: ", lxc, lyc, lzc

	sp = pf.h.sphere([xc, yc, zc], (radiusSphere, 'pc'))

	#Determine the x,y,z distances from the sink particle for each cell in the sphere.
	x = numpy.array(sp["x"].in_cgs() - xc)
	y = numpy.array(sp["y"].in_cgs() - yc)
	z = numpy.array(sp["z"].in_cgs() - zc)
	...
	if( args.magnetic) :
		Bx = numpy.array(sp["ramses_magx"])#In gauss
		By = numpy.array(sp["ramses_magy"])
		Bz = numpy.array(sp["ramses_magz"])
		Btot = numpy.sqrt(Bx*Bx + By*By + Bz*Bz)

	# grams
	cellMass = numpy.array(sp["cell_mass"])# yt now loads in g not Msolar
	# cgs
	dens = numpy.array(sp["Density"].in_cgs())#yt loads in code density
	# cm**3
	cellVolume = numpy.array(sp["cell_volume"].in_cgs())
	# These are in cm/s
	vx = numpy.array(sp["velocity_x"])
	vy = numpy.array(sp["velocity_y"])
	vz = numpy.array(sp["velocity_z"])
	...
	# Set for determination of shell in total Sphere.
...
############################################################################
	if (Bulk_by_Sphere_in_Shell): #(ShellSphere_Bulk) or (NoParticle_Bulk) or (withSmallSphere):
		# Find the Bulk Velocity of each shell via the sphere inside the shell, prior to removing from each cell in the shell
	...
	if (Bulk_by_Shell):#(Shell_Bulk):
		...
		for i in range(r.size) :
			...
			if(index >= 0 and index < bins) :
				# Get the mass of each shell
				mbin[index] = mbin[index] + cellMass[i]
				# Find the Bulk Velocity(mass weighted) of each shell
		...
		# Set the bulk velocity to be this bulk velocity
		vx_bulkvelocity_bin = vx_shell_bulk_bin/mbin
		vy_bulkvelocity_bin = vy_shell_bulk_bin/mbin
		vz_bulkvelocity_bin = vz_shell_bulk_bin/mbin
		v_vector_bulk = numpy.sqrt(vx_bulkvelocity_bin[-1]**2 + vy_bulkvelocity_bin[-1]**2 + vz_bulkvelocity_bin[-1]**2)
		print 'bulk velocity is (cm/s): ', v_vector_bulk
		print 'radius of sphere is (pc) ', radiusSphere
		print 'crossing time in sec: ', radiusSphere * parsec / v_vector_bulk
...
############################################################################
	if( args.magnetic) :
		print 'Obtaining Magnetic field data'
		timestamp_output()
...
############################################################################
	...
	for i in range(r.size) : 
		...
		index = int((math.log10(r[i]/parsec)-lgradiusMin)*bins/(lgradiusSph - lgradiusMin))
		if(index >= 0 and index < bins) :
			# Obtain a mass weighted speed out of a velocity (Mdot * distance). 
			if( args.jet) :
				# x[i],y[i],z[i] are already determined as the distance from sink to that cell i
				# and r[i] is the mag of that vector distance.
				r_hat = (x[i], y[i], z[i])/r[i] # cells x 3 matrix.# Calculate the r_hat from sink particle to each cell
			...
			mdot = vr_nomass[i] * cellMass[i]
			mdotbin[index] = mdotbin[index] + mdot/r[i]
			vrbin[index] = vrbin[index] + mdot # mass weighted r_hat velocity
...
###############################################################################
	...
	if( args.magnetic):
		...
		magnetic_fileout = fileout[:-4] + 'magnetic.out'
		numpy.savetxt(magnetic_fileout, zip(Bxbin, Bybin, Bzbin, Btotbin), fmt="%15.9E")
...
#	if True:	
	...
	if len( num_var_packed) != 0:
		...
		# Note that lx, ly and lzstar and Particle_age are in CODE UNITS!
		lxstar, lystar, lzstar, Particle_age = Obtain_Part_info_csv(sinkcsvfile)
		Particle_age = Particle_age * pf.time_unit #Convert particle_age to sec.
...
#	pf.time_unit
#	scale_t      =  0.387201000000000E+04 # Should take this from the info file rather than hardcode. TODO
	Particle_ID_list, partMass, xstar, ystar, zstar, \
	...
	if ( withFLASH4):
		...
		if( args.maxdensity):# or ( args.backwards):
			...
			if ( args.first) or ( args.second):
				...
				#TODO Figure out how this works and make it work.
		...
		for j in range(Particle_list):
			if Particle_list == 1:
				...
				if ( args.second):
					...
					#TODO Figure out the proper stepping.
					j = j + size
				if j + 1 > xstar.size:
...
# We are provided with the left and right faces of the B-field energy density data for that axis.
# We take a simple average of the two faces to obtain the avg B pressure in the center of the cell for that axis.
# We mulitply by sqrt(4*pi) as ramses is in rational units, not cgs.
# See 2012ApJS..201...24M for explanation of sqrt(4pi)
# and Finally, there is a conversion of time from scale to cgs units.

def _ramses_magx(field, data):
	return YTArray(((data["x-Bfield-left"] + data["x-Bfield-right"]) * 0.5 * \
				numpy.sqrt(4. * numpy.pi) / data.ds.time_unit), 'gauss') # Convert from rational B to cgs B.

def _ramses_magy(field, data):
	return YTArray(((data["y-Bfield-left"] + data["y-Bfield-right"]) * 0.5 * \
				numpy.sqrt(4. * numpy.pi) / data.ds.time_unit), 'gauss')

def _ramses_magz(field, data):
	return YTArray(((data["z-Bfield-left"] + data["z-Bfield-right"]) * 0.5 * \
				numpy.sqrt(4. * numpy.pi) / data.ds.time_unit), 'gauss')
...
if( args.magnetic) :
	yt.add_field(("gas", "ramses_magx"), function=_ramses_magx, units="gauss") 
	yt.add_field(("gas", "ramses_magy"), function=_ramses_magy, units="gauss") 
	yt.add_field(("gas", "ramses_magz"), function=_ramses_magz, units="gauss") 
