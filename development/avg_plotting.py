...
if args.density:
    ...
    files = ['avg_{0}_1.0Msun_shellsphere_36_sinks.txt'.format(value), 'avg_{0}_4.0Msun_shellsphere_30_sinks.txt'.format(value), \
		     'avg_density_by_hand_shellsphere_80kyr.txt', 'avg_density_by_hand_shellsphere_40kyr.txt', \
		     'avg_{0}_0.25Msun_shellsphere_6_sinks.txt'.format(value)]
    labels = ["$1.0 M_\odot$", "$4.0 M_\odot$","-80 kyr", "-40 kyr", "$0.25 M_\odot$"]
    ltypes = ["solid", "solid", "-", "-.", "solid"]
    lweights = [2, 4, 1, 1, 1]
...
if args.mdot:
    ...
    files = ['avg_{0}_1.0Msun_shellsphere_36_sinks.txt'.format(value), 'avg_{0}_4.0Msun_shellsphere_30_sinks.txt'.format(value), \
		     ...
		     'avg_mdot_by_hand_shellsphere_80kyr.txt', 'avg_mdot_by_hand_shellsphere_40kyr.txt']
...
#    continue
    ...
    if args.fit:
	    if label =='$1.0 M_\odot$' or label =='$4.0 M_\odot$':
...
#
	    ...
	    if label =='$Jet \, 1.0 M_\odot$':
		    ...
		    plt.loglog(rbin, avgbin, color='r', linewidth=lw,label="{0}".format(label),ls=ltype)
	    elif label == '$Jet \, 4.0 M_\odot$':
		    plt.loglog(rbin, avgbin, color='b', linewidth=lw,label="{0}".format(label),ls=ltype)
	    elif label == '$No \, Jet \, 4.0 M_\odot$':
		    plt.loglog(rbin, avgbin, color='b', linewidth=lw,label="{0}".format(label),ls=ltype)
	    else:
		    plt.loglog(rbin, avgbin, color='k', linewidth=lw,label="{0}".format(label),ls=ltype)
#sys.exit()
if args.density:
