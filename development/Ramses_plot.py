...
def Update_avg_value_list(filein, particle_ID, File_number, deltaM):
...
#			upper_bound = 2
			if lower_bound <= particleMass/Msun <= upper_bound:
				...
				deltaM = abs(particleMass/Msun - center_mass_4_avg)
				Update_avg_value_list(filein, particle_ID, File_number, deltaM)
			continue
...
#If we are, then we've already looped through all the files 
# and have a list of [particle_ID(int), File_number(str), deltaM(float64), quantity_2_avg_bin(np.ndarray)]
#We now need to obtain the average value.
if( args.avg) :
#	Particles_Used.append([particle_ID[0], File_number[1], deltaM[2], particleMass[3], rbin[4], vrbin[5], vrmsbin[6], vKbin[7], vmagbin[8], mTbin[9], rhobin[10], mdotbin[11], vphi_magbin[12], sum_velocities[13], vrms_r_bin[14], vrms_l_bin[15], vrms_theta_bin[16], vrms_phi_bin[17]])
	means = Obtain_avg_value()
	# perhaps break up the means into the avg_rbin etc rather than leaving as the above key.
	for i in range(len(plotting_list)):
		print plotting_list[i]
		Setup_plot_window()
		...
		if 'velocity' == plotting_list[i] :
			...
			#Toomre_Q(rbin, rhobin, vrmsbin, vphi_magbin)
			Toomre_Q(means[4], means[10], means[6], means[12])
		elif 'veldens' == plotting_list[i] :
			#Velocity_vs_Density_plot(rhobin, vrbin, vrmsbin, vKbin, vphi_magbin, particleMass, MC15_velocity)
			Velocity_vs_Density_plot(means[10], means[5], means[6], means[7], means[12], means[3][0], np.sqrt(means[6]*means[6] + means[12]*means[12]))
		elif 'density' == plotting_list[i] :
			#Density_plot(rbin, rhobin)
			...
			print "Have not yet developed averaging the various pressures."
			#Pressure_Gravity_ratio(rbin, rhobin, vrmsbin, vphi_magbin, Thermal_P_ratio, Turb_P_ratio, Rot_P_ratio)
			#Pressure_Gravity_ratio(means[4], means[10], means[6], means[12], Thermal_P_ratio, Turb_P_ratio, Rot_P_ratio)
		elif 'mass' == plotting_list[i] :
			#Total_mass_plot(rbin, mTbin)
