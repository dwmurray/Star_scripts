...
subroutine update_protostar( deltaM, deltaT, M, R, n, md, protostar_state)
  ...
  real(dp), intent( in)    :: deltaM, deltaT, M
  real(dp), intent( inout) :: R, n, md
  integer, intent( inout)          :: protostar_state
  real(dp), parameter      :: f_rad = 0.33 ! used to update to shell deuterium burning.
  real(dp)                 :: Beta_m_r, dlogBetadlogM, Beta_core_m_r, dlogBeta_core_dlogM
  real(dp)                 :: L_int, L_ion, L_D, deltaR
  real(dp)                 :: T_core
  !ZAMS checks
  real(dp)                 :: L_ZAMS, R_ZAMS, beta_ZAMS1!beta_ZAMS1 to avoid namespace confusion with parameter beta_ZAMS

103 format(3(A5,(1pe14.5,2x)))
102 format(2(A6,(1pe14.5,2x)))
101 format(1(A5,(1pe14.5,2x)))

  call init_beta_table_values
...
!        write(*,102) 'M/Msun', M/M_sun, 'R/Rsun', R/R_sun
  ...
  call obtain_Beta(n, M, R, Beta_m_r, Beta_core_m_r) !Returns Beta_m_r
  call obtain_dlogBetadlogM(n, M, R, Beta_m_r, Beta_core_m_r, &
       dlogBetadlogM, dlogBeta_core_dlogM) !Return dlogbetadlogM
...
subroutine initialize_protostar( deltaM, deltaT, m, r, n, md, T_core)
  ...
  real(dp), intent( in) :: deltaM, deltaT, m
  real(dp), intent( out) :: r, n, md, T_core
  real(dp) :: mdot
...
subroutine update_radius( deltaM, deltaT, n, m, r, Beta, dlogBetadlogM, L_int, L_ion, L_D)
  ...
  real(dp), intent( in) :: deltaM, deltaT, n, m, Beta, dlogBetadlogM, L_int, L_ion, L_D
  real(dp), intent( out) :: R !deltaR
  real(dp) :: deltaR
  real(dp) :: L_ZAMS, R_ZAMS, beta_ZAMS1 !Values from the ZAMS file. !beta_ZAMS1 to avoid conflict with constants beta_ZAMS
  real(dp) :: alpha_g! alpha_g describes the gravitational binding energy of a poltrope.
...
subroutine set_ZAMS( M_star, L_star, R_star, beta)
  ...
  real(dp), intent(out) :: L_star, R_star, beta
  real(dp) :: lgm, lgL, lgR
  integer :: i 
...
subroutine set_L_internal(M_star, R, L_int)
  ...
  real(dp), intent( in)  :: M_star, R
  real(dp), intent( out) :: L_int
  real(dp)               :: L_Hayashi, L_ZAMS, R_ZAMS, beta
...
subroutine set_L_ionize(deltaM, deltaT, L_ion)
  ...
  real(dp), intent( in)  :: deltaM, deltaT  
  real(dp), intent( out) :: L_ion
...
subroutine set_L_deuterium(n, M, R, deltaM, deltaT, protostar_state, L_int, L_ion, &
  ...
  real(dp), intent( in)   :: n, M, R, deltaM, deltaT
  real(dp), intent( in)   :: L_int, L_ion
  real(dp), intent( in)   :: Beta_m_r, Beta_core_m_r, dlogBeta_dlogM, dlogBeta_core_dlogM
  real(dp), intent( out)  :: L_D
  real(dp)                :: alpha_g! alpha_g describes the gravitational binding energy of a poltrope.
  real(dp)                :: mdot
...
subroutine determine_T_core(n, M, R, T_core)
  ...
  real(dp), intent( in)   :: n, M, R
  real(dp), intent( out)  :: T_core
  real(dp)   :: rho_core, P_core, K_value, Beta_core
  real(dp)   :: Beta_mean
  real(dp)   :: logMmin, logMmax, logRmin, logRmax
  real(dp)   :: logM, logR
  real(dp)   :: T_core_by_betacore, T_core_by_Pcore
  real(dp)   :: eps1, dphideps1, phi1
  logical            :: file_exist
...
subroutine update_md(deltaM, deltaT, L_D, md)
  use protostar_constants
  implicit none
  real(dp), intent( in) :: deltaM, deltaT, L_D
  real(dp), intent( inout) :: md
  real(dp) :: deltaMd
...
!!  real(dp), intent( in) :: n_min, n_max, M_min, M_max, logMstep, R_min, R_max, logRstep
!  real(dp)   :: n, M, R
!  real(dp)   :: eps1, dphideps1, phi1
!  real(dp)   :: rho_core, P_core, K_value, Beta_core
!  real(dp)   :: Beta_mean
!  real(dp)   :: logMmin, logMmax, logRmin, logRmax
!  real(dp)   :: logM, logR
!  real(dp)   :: T_core_by_betacore, T_core_by_Pcore, R_star_check
!  integer            :: i
!  logical            :: file_exist
!
...
subroutine determine_Beta_mean(n, eps1, M_star, R_star, rho_core, P_core, K_value, Beta_core, &
  ...
  real(dp), intent( in)  :: n, eps1, M_star, R_star, rho_core, P_core, K_value, Beta_core
  real(dp), intent( out) :: Beta_mean
  real(dp) :: eps_r, dphideps_r, phi_r
  real(dp) :: eps, deps, phi, dphideps, r_0
  real(dp) :: rho_r, P_r, Beta_r, r, dr
  real(dp) :: Pressure_Beta_Integral, Pressure_Integral
  integer          :: i
...
subroutine init_beta_table_values
  use protostar_constants
  use protostar_parameters
  use rk2_polytrope
  use root_find
  ...
  real(dp)    :: n, M, R, rho_core, P_core, K_value, Beta_core, Beta_mean
  real(dp)    :: eps1, dphideps1, phi1
  real(dp)    :: logMmin, logMmax, logRmin, logRmax
  real(dp)    :: logM, logR
  real(dp)    :: T_core_by_betacore, T_core_by_Pcore, R_star_check
  ...
  if( initialized_protostar) then
     return
  end if 
  initialized_protostar = .true.
 
  beta_table_dim1 = 1 + n_step * (log10(M_max) - log10(M_min))/logM_step * (log10(R_max) - log10(R_min))/logR_step

  allocate(Beta_file_table(beta_table_dim1,beta_table_dim2))
...
subroutine obtain_Beta(n_in, M_in, R_in, Beta_m_r, Beta_core_m_r)
  use protostar_constants
  use protostar_parameters
  implicit none

  real(dp), intent( in)                :: n_in, M_in, R_in
  real(dp), intent( out)               :: Beta_m_r, Beta_core_m_r
  real(dp)                             :: n, M, R, rho_core, P_core, Beta_core, Beta_mean
  real(dp)                             :: logM_in, logR_in, logBeta_m_r, logBeta_core_m_r
  integer                                      :: line, index
  integer, parameter                           :: nvars = 2
  real(dp), dimension(nvars, nvars)    :: n_bracket, M_bracket, R_bracket
  real(dp), dimension(nvars, nvars)    :: rho_core_bracket, P_core_bracket, Beta_core_bracket
  real(dp), dimension(nvars, nvars)    :: Beta_mean_bracket
...
subroutine obtain_dlogBetadlogM(n_in, M_in, R_in, Beta_m_r, Beta_core_m_r, &
     dlogBetadlogM, dlogBeta_coredlogM)
  use protostar_constants
  use protostar_parameters
  implicit none

  real(dp), intent( in)               :: n_in, M_in, R_in, Beta_m_r, Beta_core_m_r
  real(dp), intent( out)              :: dlogBetadlogM, dlogBeta_coredlogM
  !for obtaining dlogBeta/dlogM
  real(dp)                            :: n_prime, M_prime, R_prime, Beta_prime_m_r, Beta_prime_core_m_r
  real(dp)                            :: dlogBeta, dlogBeta_core, dlogM
  ...
  call obtain_Beta(n_prime, M_prime, R_prime, Beta_prime_m_r, Beta_prime_core_m_r)
...
subroutine interpolate(nvars, logM_in, logR_in, M_bracket, R_bracket, X_bracket,  logX_m_r)
  ...
  real(dp), intent( in), dimension(nvars, nvars) :: M_bracket, R_bracket, X_bracket!these are in log space
  real(dp), intent( in)   :: logM_in, logR_in
  real(dp), intent( out) :: logX_m_r
  real(dp)               :: delta_logm, delta_logr
  real(dp)               :: logX_m_r1, logX_m_r2
