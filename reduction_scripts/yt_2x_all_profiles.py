...
############################################################################
	if (ShellSphere_Bulk) or (NoParticle_Bulk) or (withSmallSphere):
		...
		print 'bulk velocity is: ', v_vector_bulk
...
#####################################################################
	...
	for i in range(r.size) : 
		...
		if(index >= 0 and index < bins) :
			...
			#vrms_radial = np.sqrt(vrms_radial_x**2 + vrms_radial_y**2 + vrms_radial_z**2) * cellMass[i]
			vrms_radial = (vrms_radial_x + vrms_radial_y + vrms_radial_z)

			#sin_theta_ = np.sqrt(rad**2 - z**2)/rad
			vrms_theta_x = vrms_x * (x[i]*z[i])/(rad*np.sqrt(rad**2 - z[i]**2))
			vrms_theta_y = vrms_y * (y[i]*z[i])/(rad*np.sqrt(rad**2 - z[i]**2))
			vrms_theta_z = vrms_z * -1.0 * np.sqrt(rad**2 - z[i]**2)/rad
			#vrms_theta = np.sqrt(vrms_theta_x**2 + vrms_theta_y**2 + vrms_theta_z**2) * cellMass[i]
			vrms_theta = (vrms_theta_x + vrms_theta_y + vrms_theta_z)

			vrms_phi_x = vrms_x * -1.0 * y[i]/np.sqrt(rad**2 - z[i]**2)
			vrms_phi_y = vrms_y * x[i]/(np.sqrt(rad**2 - z[i]**2))
			vrms_phi_z = 0.0
			#vrms_phi = np.sqrt(vrms_phi_x**2 + vrms_phi_y**2 + vrms_phi_z**2) * cellMass[i]
			# Can't sum in quad?
			vrms_phi = (vrms_phi_x + vrms_phi_y + vrms_phi_z)
			# avg out using the abs for phi and theta
			#vrms_phi = (vrms_phi)
			#vrms_lateral = np.sqrt(vrms_theta**2 + vrms_phi**2)
	...
	vrms_r_bin = np.sqrt(vrms_r_bin/mbin)
	#vrms_l_bin = vrms_l_bin/mbin
	vrms_theta_bin = np.sqrt(vrms_theta_bin/mbin)
	vrms_phi_bin = np.sqrt(vrms_phi_bin/mbin)
...
#		particle_age = particle_age / (np.pi*1e7)
#
	...
	#numpy.savetxt(fileout, zip(rbin, vrbin, vrmsbin, vrmsnbin, vKbin, vmagbin, vmagnbin, mTbin, rhobin, mdotbin, norm, angXbin, angYbin, angZbin, vphi_magbin, sum_velocities, store_particle_mass, store_part_creation_time, store_current_time, Number_of_cells_bin), fmt="%15.9E")
################################################
...
def standardize_file_number(input_file_number):
	if len(str(input_file_number)) == 2:
		...
		print 'We"ve gone above ten thousand files apparently.'
		sys.exit()
	return output_file_number

def file_existance_chk(file_value):
...
def obtain_individual_part_atributes(part_lst_num, file_number, pf, particle_ID, xp, yp, zp, vxp, vyp, vzp, partMass, creation_time, current_time):
	xc = xp[part_lst_num]	 
	yc = yp[part_lst_num]	 
	zc = zp[part_lst_num]
...
def pass_to_analysis_script(pf, xc, yc, zc, this_particle_id, this_creation_time, current_time, radiusMin, radiusSphere, particleMass, file_number):
	...
	txt_filename = 'particle_{0}_location.txt'.format(int(withParticleIDValue))
	print_to_text_file(txt_filename, file_number, this_particle_id, xc, yc, zc, this_creation_time, current_time, particleMass)
...
#					    'a list of particles that are in this quadrant.'
				...
				print 'On particle:', j + 1, 'of:', xp.size, ' in File: {0}{1:04d}'.format(plt_prefix, i)
				print 'Particle Mass is', partMass[j]
				pass_to_analysis_script(pf, xc, yc, zc, this_particle_id, this_creation_time, current_time, radiusMin, radiusSphere, particleMass, file_number)
...
#############################################################
		...
		if not (particle_exists_here):
			if withRestart:
				loaded_file_number, loaded_ParticleIDValue, loaded_xc_search, loaded_yc_search, loaded_zc_search, loaded_creation_time, loaded_particle_mass, loaded_current_time = np.loadtxt("particle_{0}_location.txt".format(withParticleIDValue), unpack=True)
				for value in range(len(loaded_file_number)):
					...
					pulled_file_number = standardize_file_number(str(loaded_file_number[value])[:-2])
			...
			pass_to_analysis_script(pf, xc_new, yc_new, zc_new, this_particle_id, this_creation_time, current_time, radiusMin, radiusSphere, particleMass, file_number)
...
def Main_tracing_forwards():
	for i in range(args.start,args.end,args.step) :	
		...
		with open("particle_location.txt", "a") as particle_location:
			...
			particle_location.write('\n')
			particle_location.close()
		half_point = xp.size/2
		if (withFirstHalf):
			print 'will only do the first half of the list of particles.'
			for j in range(xp.size/2 + 1):
				part_lst_num = j
				print 'On particle:', part_lst_num + 1, 'of:', xp.size, ' in File: {0}{1:04d}'.format(plt_prefix, i)
...
# or have issues with np.linalg.inv
