...
#####################################
	...
	for i in range(r.size) : 
		...
		if(index >= 0 and index < bins) :
			...
			# Why no mass weighting here?
			vxbin[index] = vxbin[index] + vx[i]
			#vz_bulk_bin[index] = (vz[i]*cellMass[i]).sum()/mbin[index]
			#vz_bulk_bin[index] = vz_bulk_bin[index] + (vz[i]*cellMass[i])/mbin[index]
			#print vx_bulk_bin[-1], vx[i]*cellMass[i]/mbin[index]

	for shell_test in range(bins):
		vx_bulk_bin[shell_test] = vx_bulk_bin[shell_test]/mbin[shell_test]
		vy_bulk_bin[shell_test] = vy_bulk_bin[shell_test]/mbin[shell_test]
		vz_bulk_bin[shell_test] = vz_bulk_bin[shell_test]/mbin[shell_test]
	print vx_bulk_bin
	print vxbin
	print vx_bulk_bin[1], vxbin[1]
	print vx_bulk_bin[-1], vxbin[-1]
	print mbin
	vxbin = numpy.zeros(bins)
	#mbin = numpy.zeros(bins)
#####################################
	...
	for i in range(r.size) : 
		...
		if(index >= 0 and index < bins) :
			# Get the mass of each shell
			# Calculated above as needed for bulk shell velocity
			#mbin[index] = mbin[index] + cellMass[i]
			# and the Volume of each shell
			volbin[index] = volbin[index] + cellVolume[i]

#			# Find the Bulk velocity of each shell
#			vx_bulk_bin[index] = (vx[i]*cellMass[i]).sum()/mbin[index]
#			vy_bulk_bin[index] = (vy[i]*cellMass[i]).sum()/mbin[index]
#			vz_bulk_bin[index] = (vz[i]*cellMass[i]).sum()/mbin[index]
#			
#			print vx_bulk_bin[-1]
#
			# Find the velocity of the shell
			vxbin[index] = vxbin[index] + vx[i]
			vybin[index] = vybin[index] + vy[i]
			vzbin[index] = vzbin[index] + vz[i]

#			print vxbin
			# Subtract the bulk velocity from each shell
			vxbin[index] = vxbin[index] - vx_bulk_bin[index]
			vybin[index] = vybin[index] - vy_bulk_bin[index]
			vzbin[index] = vzbin[index] - vz_bulk_bin[index]

			# This vr is actually Mdot * distance
			vr_x_mass = ((vx[i] - vx_bulk_bin[index])*x[i] + (vy[i] - vy_bulk_bin[index])*y[i] + (vz[i] - vz_bulk_bin[index])*z[i])*cellMass[i]/r[i]

			#vr = (abs(vx[i]*x[i]) + abs(vy[i]*y[i]) + abs(vz[i]*z[i]))*cellMass[i]/r[i]
