...
def project( filein, xc, yc, zc, fileout = 'junk.png', withParticles=False) :
	print moviefile
	print xc
	print yc
	print zc
	p = ProjectionPlot(pf, "z", "Density", center = (xc, yc, zc), width = (1.0, 'pc'))
...
for i in range(args.start,args.end) :	
	...
	print file
	moviefile = "movie_{0}_frame_zoom_{1:04d}.png".format(quad, i)
	pf = load(file)
	dd = pf.h.all_data()
	xp = dd["particle_posx"]
	yp = dd["particle_posy"]
	...
	partMass = dd["ParticleMassMsun"]
        ...
        for j in range(xp.size) :
                ...
                yc = yp[j]
                zc = zp[j]
		moviefile = "movie_{0}_frame_zoom_{1:04d}_part_{2:03d}.png".format(quad, i,j)
		project( file, xc, yc, zc, moviefile, withParticles=args.particle)
