...
def getRadialProfileSimple(pf, xc, yc, zc, vxc, vyc, vzc, fileout="rad_profile.out", radiusMin=1e-3, radiusSphere=3.0, particleMass = 0.0) : 
...
def getRadialProfile(pf, xc, yc, zc, fileout="rad_profile.out", radiusMin=1e-3, radiusSphere=3.0, particleMass = 0.0) : 
	...
	Mtot = cellMass.sum()
	#This is the mass weighted bulk velocity
	# This is the velocity of the entire sphere
...
#
	...
	sp2 = pf.h.sphere([xc,yc,zc], 0.005/pf['pc'])
	vcx = sp2["x-velocity"]
	vcy = sp2["y-velocity"]
	vcz = sp2["z-velocity"]
...
#	
	...
	for i in range(r.size) : 
		...
		if(index >= 0 and index < bins) :
			...
			# Should be dividing by r**2 for mdot?
...
for i in range(args.start,args.end,args.step) :	
	...
	for j in range(xp.size) :
		...
		#getRadialProfile(pf,xc,yc,zc, fileout="{0}{1:04d}_part_{2:03d}.out".format( out_prefix, i, j), radiusSphere=4.0, particleMass = partMass[j])
		getRadialProfileSimple(pf,xc,yc,zc, vxc, vyc, vzc, fileout="{0}{1:04d}_part_{2:03d}.out".format( out_prefix, i, j), radiusSphere=3.0, particleMass = partMass[j])
