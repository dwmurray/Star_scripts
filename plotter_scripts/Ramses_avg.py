...
#	index_turb_min = index_turb_min + 20
	...
	#pl.loglog( rbin, sum_of_velocities/1e5, 'c', label='$\sqrt{v_r^2 + v_T^2 + v_\phi^2}$')  
...
def disk_radius_by_vphi(rbin, vrbin, vrmsbin, vphi_magbin):
	...
	if last_r_index < last_t_index:
		...
		print 'seems like there is no disk by this method'
		disk_radius_index = 0
	print 'This is rbin[disk_radius_index] in pcs', rbin[disk_radius_index]

#	return disk_radius_index
...
def png_save(fileout):
	print 'saving file: ', fileout
        #pl.savefig('Ramses_velocity_avg.pdf')
	pl.savefig(fileout)
...
def disperse_2_plotters(quad, framestep, compare_files, particle_number, output_format, rbin, vrbin, vrmsbin, vrmsnbin, vKbin, vmagbin, vmagnbin, mTbin, rhobin, mdotbin, norm, angXbin, angYbin, angZbin, vphi_magbin, sum_of_velocities, partMass, part_creation_time, current_time, vrms_r_bin, vrms_l_bin, vrms_theta_bin, vrms_phi_bin, all_independent, velocity_alone, Toomre_Q_alone, density_alone, angmv_alone, mass_alone, mdot_alone, veldens_alone, Pressure_alone):
	...
	if (all_independent):
		...
		Pressure_alone = True
	if (velocity_alone):
		fileout = "velocity_{0}_{1}_{2}{3}.{4}".format(quad, framestep, compare_files, particle_number, output_format)
		velocity_plot(rbin, vrbin, vrmsbin, vrmsnbin, vKbin, vmagbin, vmagnbin, mTbin, vphi_magbin)
		png_save(fileout)
		fileout = "Turbulent_{0}_{1}_{2}{3}.{4}".format(quad, framestep, compare_files, particle_number, output_format)
		Turbulent_velocity_plot(rbin, vrbin, vrmsbin, vrmsnbin, vKbin, vmagbin, vmagnbin, mTbin, vphi_magbin)
		png_save(fileout)
	if (Toomre_Q_alone):
		fileout = "Toomre_Q_{0}_{1}_{2}{3}.{4}".format(quad, framestep, compare_files, particle_number, output_format)
		Toomre_Q(rbin, rhobin, vrbin, vrmsbin, vphi_magbin)				
		png_save(fileout)
	if (veldens_alone):
		fileout = "vel_dens_{0}_{1}_{2}{3}.{4}".format(quad, framestep, compare_files, particle_number, output_format)
		velocity_vs_density_plot(rbin, vrbin, vrmsbin, vrmsnbin, vKbin, vmagbin, vmagnbin, mTbin, vphi_magbin, rhobin)
		png_save(fileout)
	if (density_alone):
		fileout = "density_{0}_{1}_{2}{3}.{4}".format(quad, framestep, compare_files, particle_number, output_format)
		density_plot(rbin, mTbin, rhobin)
		png_save(fileout)
	if (Pressure_alone):
		fileout = "Pressure_{0}_{1}_{2}{3}.{4}".format(quad, framestep, compare_files, particle_number, output_format)
		Pressure_Gravity_ratio(rbin, rhobin, vrmsbin, mTbin)
		png_save(fileout)
	if (angmv_alone):
		fileout = "angmomentum_{0}_{1}_{2}{3}.{4}".format(quad, framestep, compare_files, particle_number, output_format)
		ang_moment_plot(rbin, mTbin, vphi_magbin)
		png_save(fileout)
	if (mass_alone):
		fileout = "mass_{0}_{1}_{2}{3}.{4}".format(quad, framestep, compare_files, particle_number, output_format)
		total_mass_plot(rbin, mTbin)
		png_save(fileout)
	if (mdot_alone):
		fileout = "mdot_{0}_{1}_{2}{3}.{4}".format(quad, framestep, compare_files, particle_number, output_format)
...
parser.add_argument('avgMass', metavar='N4', type=int, help='Must provide Target Mass to avg the values around')
...
# Attempting to set up avgs.
total_particle_count = 0.0
disk_particle_mass_sum = 0.0
rbin_sum = 0.0
vrbin_sum = 0.0
vrmsbin_sum = 0.0
vrmsnbin_sum = 0.0
...
angZbin_sum = 0.0
vphi_magbin_sum = 0.0
sum_of_velocities_sum = 0.0
partMass_sum = 0.0
vrms_r_bin_sum = 0.0
vrms_l_bin_sum = 0.0
vrms_theta_bin_sum = 0.0
vrms_phi_bin_sum = 0.0
total_particle_count = 0.0
...
#	file_exist = glob.glob('{0}_{1:04d}_{2}*.out'.format(file_prefix, framestep, compare_files))
#	if not file_exist:
#		print 'File: "{0}_{1:04d}_{2}*.out" does not exist!'.format(file_prefix, framestep, compare_files)
#		continue
	# If the file exists, 
...
#		print type(particle_list)
	...
	for j in range(len(particle_list)):
		...
		if particle_number == withParticleIDValue or (withAllParticles):
			...
			print 'Checking if in required range'
                        # partMass is an array filled identically with the particle Mass.
                        if (args.avgMass - args.avgMass*0.1) <= partMass[0] and partMass[0] <=  (args.avgMass + args.avgMass*0.1):
                            print 'Within 10% of ', args.avgMass, 'adding it to the profile.'
                        else:
                            print 'particle mass is not within 10% of ', args.avgMass, ' solar masses.'
                            print 'This is the mass of the particle:', partMass[0]
                            continue
			...
			r_star = 0.0
                        disk_particle_mass_sum += disk_particle_mass 
                        rbin_sum += rbin
                        vrbin_sum += vrbin
                        vrmsbin_sum += vrmsbin
                        vrmsnbin_sum += vrmsnbin
                        vKbin_sum += vKbin
                        vmagbin_sum += vmagbin
                        vmagnbin_sum += vmagnbin
                        mTbin_sum += mTbin
                        rhobin_sum += rhobin
                        mdotbin_sum += mdotbin
                        angXbin_sum += angXbin
                        angYbin_sum += angYbin
                        angZbin_sum += angZbin
                        vphi_magbin_sum += vphi_magbin
                        sum_of_velocities_sum += sum_of_velocities
                        partMass_sum += partMass
                        vrms_r_bin_sum += vrms_r_bin
                        vrms_l_bin_sum += vrms_l_bin
                        vrms_theta_bin_sum += vrms_theta_bin
                        vrms_phi_bin_sum += vrms_phi_bin
                        total_particle_count += 1.

                        print 'now avg over all particles accepted thus far.'
                        disk_particle_mass_avg = disk_particle_mass_sum / total_particle_count
                        rbin_avg = rbin_sum / total_particle_count
                        vrbin_avg = vrbin_sum / total_particle_count
                        vrmsbin_avg = vrmsbin_sum / total_particle_count
                        vrmsnbin_avg = vrmsnbin_sum / total_particle_count
                        vKbin_avg = vKbin_sum / total_particle_count
                        vmagbin_avg = vmagbin_sum / total_particle_count
                        vmagnbin_avg = vmagnbin_sum / total_particle_count
                        mTbin_avg = mTbin_sum / total_particle_count
                        rhobin_avg = rhobin_sum / total_particle_count
                        mdotbin_avg = mdotbin_sum / total_particle_count
                        angXbin_avg = angXbin_sum / total_particle_count
                        angYbin_avg = angYbin_sum / total_particle_count
                        angZbin_avg = angZbin_sum / total_particle_count
                        vphi_magbin_avg = vphi_magbin_sum / total_particle_count
                        sum_of_velocities_avg = sum_of_velocities_sum / total_particle_count
                        partMass_avg = partMass_sum / total_particle_count
                        vrms_r_bin_avg = vrms_r_bin_sum / total_particle_count
                        vrms_l_bin_avg = vrms_l_bin_sum / total_particle_count
                        vrms_theta_bin_avg = vrms_theta_bin_sum / total_particle_count
                        vrms_phi_bin_avg = vrms_phi_bin_sum / total_particle_count

# Having cycled through all framesteps and particles and averaged everything, now plot it.
                        framestep1 = 'avg'
                        particle_number1 = 'avg'

                        disperse_2_plotters(quad, framestep1, compare_files, particle_number1, output_format, rbin_avg, vrbin_avg, vrmsbin_avg, vrmsnbin_avg, vKbin_avg, vmagbin_avg, vmagnbin_avg, mTbin_avg, rhobin_avg, mdotbin_avg, norm, angXbin_avg, angYbin_avg, angZbin_avg, vphi_magbin_avg, sum_of_velocities, partMass, part_creation_time, current_time, vrms_r_bin_avg, vrms_l_bin_avg, vrms_theta_bin_avg, vrms_phi_bin_avg, all_independent, velocity_alone, Toomre_Q_alone, density_alone, angmv_alone, mass_alone, mdot_alone, veldens_alone, Pressure_alone)
